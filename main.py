...
def event_acc(toggle, html):
    if toggle:
        ...
        shots = scored + missed
        acc = round(scored / shots, 3)
        # print('\n\nscored:',scored,'\nmiss_amp:',miss_amp,'\nmiss_speaker:',miss_speaker,'\nmissed:',missed,'\nshots:',shots,'\nacc:',acc,'\nprint:',acc * 100,'\n\n')
        if html: return f'<h4>Teams at this event have scored</h4><h3>{round(acc * 100, 1)}%</h3><h4>of total notes shot!</h4>'
        else: return acc
    else: return 'bad'
def chicken_foul():
