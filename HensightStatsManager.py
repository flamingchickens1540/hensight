...
class HensightStatsManager:
    ...
    def update_stats(self):
        ...
        for matches in self.data.event_to_match_data.values():
            if type(matches) != MatchData:
                for match_data in matches.values():
                    ...
                    new_auto_notes = (
                        match_data.autoAmpNotes + match_data.autoSpeakerNotes
                    )
                    global_auto_notes = global_auto_notes + new_auto_notes
                    global_tele_notes = global_tele_notes + (
                        match_data.totalNotes - new_auto_notes
                    )
                    global_trap_notes = global_trap_notes + match_data.trapNotes
                    global_spotlight_notes = (
                        global_spotlight_notes + match_data.numberOfSpotlights
                    )
                    global_speaker_notes = (
                        global_speaker_notes + match_data.totalSpeakerNotes
                    )
                    global_amplified_speaker_notes = (
                        global_amplified_speaker_notes
                        + match_data.amplifiedSpeakerNotes
                    )
                    global_amp_notes = global_amp_notes + match_data.totalAmpNotes
                    global_total_notes = global_total_notes + match_data.totalNotes
                    global_distance_traveled = (
                        global_distance_traveled + match_data.totalDistanceTraveled
                    )
                    global_red_alliance_score = (
                        global_red_alliance_score + match_data.redScore
                    )
                    global_blue_alliance_score = (
                        global_blue_alliance_score + match_data.blueScore
                    )
        ...
        for match_data in self.current_event_data.values():
            ...
            event_tele_notes = event_tele_notes + (
                match_data.totalNotes - new_auto_notes
            )
            event_trap_notes = event_trap_notes + match_data.trapNotes
            event_spotlight_notes = (
                event_spotlight_notes + match_data.numberOfSpotlights
            )
            event_speaker_notes = event_speaker_notes + match_data.totalSpeakerNotes
            event_amplified_speaker_notes = (
                event_amplified_speaker_notes + match_data.amplifiedSpeakerNotes
            )
            ...
            event_distance_traveled = (
                event_distance_traveled + match_data.totalDistanceTraveled
            )
        ...
        for matches in self.current_events_data:
            if type(matches) != MatchData:
                for match_data in matches.values():
                    ...
                    current_tele_notes = current_tele_notes + (
                        match_data.totalNotes - new_auto_notes
                    )
                    current_trap_notes = current_trap_notes + match_data.trapNotes
                    current_spotlight_notes = (
                        current_spotlight_notes + match_data.numberOfSpotlights
                    )
                    current_speaker_notes = (
                        current_speaker_notes + match_data.totalSpeakerNotes
                    )
                    current_amplified_speaker_notes = (
                        current_amplified_speaker_notes
                        + match_data.amplifiedSpeakerNotes
                    )
                    ...
                    current_distance_traveled = (
                        current_distance_traveled + match_data.totalDistanceTraveled
                    )
                    current_red_alliance_score = (
                        current_red_alliance_score + match_data.redScore
                    )
                    current_blue_alliance_score = (
                        current_blue_alliance_score + match_data.blueScore
                    )
                    if match_data.melodyRP != 0:
        ...
        self.hensight_stats["global_spotlight_note_percentage"] = (
            global_spotlight_notes / (6 * global_number_of_matches)
        )
        if current_number_of_matches != 0:
            self.hensight_stats["event_spotlight_note_percentage"] = (
                event_spotlight_notes / (6 * event_number_of_matches)
            )
        ...
        self.hensight_stats["global_amplified_speaker_notes"] = (
            global_amplified_speaker_notes
        )
        self.hensight_stats["current_amplified_speaker_notes"] = (
            current_amplified_speaker_notes
        )
        self.hensight_stats["event_amplified_speaker_notes"] = (
            event_amplified_speaker_notes
        )
