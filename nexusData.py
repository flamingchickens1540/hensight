...
def nexusData():
  ...
  if my_next_match:
    ...
    if time_until_queue > 60 and time_until_queue < 3600: 
      formated_time = "and will queue in "+str(round(time_until_queue/60))+" minutes"
    elif time_until_queue > 3600 and time_until_queue: 
      formated_time = "and will queue in "+str(round(time_until_queue/3600,1))+" hour(s)"
    elif time_until_queue < 60 and time_until_queue > 0: formated_time = 'and will queue in '+str(round(time_until_queue))+' secconds'
    else: formated_time = 'is queueing NOW'
    ...
    matchInfo = "Team {} doesn't have any future matches scheduled yet".format(my_team_number)
  ...
  for announcement in data['announcements']:
    announcements.append("Event announcement: {}".format(announcement['announcement']))
  pulseData.append(announcements)

  for parts_request in data['partsRequests']:
    partRequests.append("Parts request for team {}: {}".format(parts_request['requestedByTeam'], parts_request['parts']))
  pulseData.append(partRequests)

  # pulseData.append(data['nowQueuing'])
  # if hasattr(data, 'nowQueuing'):
  #   print('--- Queuing a match!!')
  #   pulseData.append(data['nowQueuing'])
  # else: pulseData.append('')
  
  if 'nowQueuing' in data: pulseData.append(data['nowQueuing'])
  else: pulseData.append('')
  
  my_upcomming_matches = []
  for i in data['matches']:
    if i['status'] == 'On field': continue
    if my_team_number in i['redTeams'] or my_team_number in i['blueTeams']:  my_upcomming_matches.append(i)
