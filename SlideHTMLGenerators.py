...
def feather_message(toggle):
    if toggle:
        return "<h4>There are as many people that do FIRST as feathers on </h4><h2 style='font-size: 9rem;'>Ten and Half chickens!</h2>"
    else:
        return 'bad'
...
def event_total_notes(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h4>There have been</h4><h3>{"{:,}".format(round(hensightStats.hensight_stats["event_total_notes"]))}</h3><h4>notes scored in Galileo</h4>'
    else:
        return 'bad'
def event_trap_notes(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h4>Teams have scored</h4><h3>{round(hensightStats.hensight_stats["event_trap_notes"])}</h3><h4>notes in the trap in Galileo</h4>'
    else:
        return 'bad'
...
def event_high_score(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h4>The high score in Galileo is</h4><h3>{round(hensightStats.hensight_stats["event_high_score"])}</h3><h4>points! (good job)</h4>'
    else:
        return 'bad'
...
def event_auto_notes(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h4>Teams have scored </h4><h2>{"{:,}".format(round(hensightStats.hensight_stats["event_auto_notes"]))}</h2><h4>notes in auto in Galileo</h4>'
    else:
        return 'bad'
...
def event_tele_notes(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h4>Teams have scored </h4><h2>{"{:,}".format(round(hensightStats.hensight_stats["event_tele_notes"]))}</h2><h4>notes in tele in Galileo</h4>'
    else:
        return 'bad'
...
def spotlight_percent(toggle, event_toggle):
    if toggle and event_toggle:
        return f'<h1>In Galileo human players hit</h1><h3>{round(hensightStats.hensight_stats["event_spotlight_note_percentage"], 2)}%<h3><h1>of high notes </h1><div></div><h1>In all of worlds human players hit</h1><h3>{round(hensightStats.hensight_stats["current_spotlight_note_percentage"], 2)}%</h3><h1>of high notes</h1>'
    else:
        return 'bad'
...
def global_travel(toggle):
    if toggle:
        ...
        return 'bad'
print('--global melody rp: ',hensightStats.hensight_stats["global_melody_rp"])
...
def global_ensemble_rp(toggle):
    if toggle:
        return f'<h4>The Ensemble RP was achieved in </h4><h3>{"{:,}".format(round(hensightStats.hensight_stats["global_ensemble_rp"], 2))}%</h3><h4>of matches this year</h4>'
    else:
        return 'bad'
